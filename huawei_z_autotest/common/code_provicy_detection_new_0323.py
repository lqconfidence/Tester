...
# 关键词
sed_word_list = ['个人信息', '手机', '授权', '法律法规', '收集', '数据', '权限', '账户', '合法权益', '权利', '披露', '政策', '保障', '存储', '储存', '条款', '身份验证', '服务器', '隐私权', '共享', 'cookie', 'Cookie', '身份认证', '儿童', '设备识别码', 'IP', 'GPS', 'IMEI', '隐私', '隐私保护', '隐私声明', '隐私政策', '个人隐私']
        ...
        with open(file_path, encoding='utf-8', mode='r')as rd:
            ...
            if len(text) > 0:
                ...
                if count_ >= 7:
                    write_ = True
                if write_:
                    with open('D:\\PycharmProjects\\privacy\\' + file_path.split('\\')[-1],
                              encoding='utf-8', mode='a+')as wt:
                        wt.write(text)
                else:
                    with open('D:\\PycharmProjects\\notprivacy\\' + file_path.split('\\')[-1],
