...
class TestInnerIssueAPI(unittest.TestCase):
    ...
    @ddt.data(*test_datas)
    def test_hw_pp(self, test_data):
        mylog.info('test data{}'.format(test_data))

        method = "POST"
        trace_id = test_data['trace_id']
        expected_case = test_data['expected_case']
        expected_is_privacy = test_data['expected_is_privacy']
        ...
        excel_hw_privacyreview_case.write(sheet_name, test_data['case_id'] + 1,
                                    sheet_head.__getattribute__("prv_is_privacy"),str(prv_is_privacy))
        print(expected_is_privacy, type(expected_is_privacy), prv_is_privacy['state'], type(prv_is_privacy['state']))
        if expected_is_privacy == str(prv_is_privacy['state']):
            excel_hw_privacyreview_case.write(sheet_name, test_data['case_id'] + 1,
                                         sheet_head.__getattribute__("judge_privacy"), data='pass')
        else:
            excel_hw_privacyreview_case.write(sheet_name, test_data['case_id'] + 1,
                                         sheet_head.__getattribute__("judge_privacy"), data='fail')
            excel_hw_privacyreview_case.write(sheet_name, test_data['case_id'] + 1,
        ...
        if prv_policy_content_data != None:
            for i in  prv_policy_content_data:
                #print(i)
                if i["type"] == sheet_name:
                    #print(i["data"])
                    excel_hw_privacyreview_case.write(sheet_name, test_data['case_id'] + 1, sheet_head.__getattribute__(sheet_name),
                                      "state:{}".format(i["state"])+"--data:{}".format(str(i["data"])))
                    print(expected_case,type(expected_case), i['state'], type(i['state']))
                    if expected_case == str(i['state']):
                        excel_hw_privacyreview_case.write(sheet_name, test_data['case_id'] + 1,
                                                     sheet_head.__getattribute__('case_result'),data="pass")
                    else:
                        excel_hw_privacyreview_case.write(sheet_name, test_data['case_id'] + 1,
                                                     sheet_head.__getattribute__('case_result'), data="fail")
        else:
            pass
