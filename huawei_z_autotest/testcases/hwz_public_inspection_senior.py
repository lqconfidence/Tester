...
def request_t(url,request_body,headers):
...
for i in range(0, mod * step, step):
    for j in range(i, i + step):
        ...
        try:
            ...
            # res = requests.post(url=url, json=request_body,headers=headers)
            t=threading.Thread(target = request_t,args=(url,request_body,headers))
            t.daemon = True
            print("当前线程数为：{}".format(threading.active_count()))
            print("当前线程名为：{}".format(threading.current_thread().name))
            print("当前线程标识为：{}".format(threading.current_thread().ident))
            t.start()
            time.sleep(2)
